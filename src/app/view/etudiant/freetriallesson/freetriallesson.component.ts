...
export class FreetriallessonComponent implements OnInit {
  ...
  constructor(public loginService: LoginService, public freetrialService: FreeTrialServiceService, private messageService: MessageService, private userService: UserService, private inscriptionService: InscriptionService) {
    this.daysOptions = [
      { name: '1 day', value: 1 },
  ...
  ngOnInit() {
    ...
    this.inscriptionService.findByEtudiantId(this.loginService.getConnectedStudent().id).subscribe(
        data => {
          this.freetrialService.freeTrialFormule.inscription = data;
        }
    );
