...
export class QuizTakeComponent implements OnInit {
    ...
    textSeleted: string;
    answersList: Map<number, Array<Reponse>> = new Map<number, Array<Reponse>>();
    ...
    previousQuestion: Question = new Question();
    showCheckButton = true;
    showHideButton = false;
    showShowedButton = false;
    showAnswerDiv = false;
    ...
    ngOnInit(): void {
        let listRep: Array<Reponse> = new Array<Reponse>();
        this.numQuestion = 0;
        this.noteQuiz = 0;
    ...
    //         data => {
    //             this.questionList = data;
    //             if (this.numQuestion > this.questionList.length) {
    ...
    //         }
    //     );
    //     if (this.numQuestion > this.questionList.length && this.numQuestion > 1) {
    ...
    filterDict($event: any) {

    }

    saveAnswers(question: Question) {

        if ( question.typeDeQuestion.ref === 't1' || question.typeDeQuestion.ref === 't3' ){
            this.multiChoiceAnswer.push({... this.answer});
        }
        console.log(this.multiChoiceAnswer);
        this.showHideButton = true;
        this.showCheckButton = false;
        this.showAnswerDiv = true;
        this.answersList.set(question.id, this.multiChoiceAnswer);
        this.disabledButtonCheck = true;
    ...
    nextQuestionFct() {
        ...
        if (this.answersList.get(this.question.id) === undefined) {
            this.showShowedButton = false;
